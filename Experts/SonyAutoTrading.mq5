//+------------------------------------------------------------------+
//| TestLayersEA.mq5  (Minimal test version)                         |
//+------------------------------------------------------------------+
#property strict
#property version "1.00"

// Log history to server (Commented out for compilation)
// //---
...
// API only (Commented out for compilation)
// //---
...
//+------------------------------------------------------------------+
//| Collects statistics on open positions with the EA's magic number |
//+------------------------------------------------------------------+
...
void ManageLayers() {
   ...
   // Just prevent the douplicate create Position
...
// If no positions are open, start a new sequence
   if(s.count == 0) {
      if(AutoStartOnInit && !sequence_active) {
         // Update lock time before initial buy
         last_trade_time = TimeCurrent();
         OpenBuy(BaseLots);
      }
      return;
...
// Step 2: Get the last entry price and current bid price
   last_entry_price = s.lastPrice;
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);

// Step 3: Define your fixed adverse distance (as requested: $5)
// StepPerLayer is between Example 10 -> 5
   double adverse_threshold = StepsPerLayer;

// Step 4: Calculate the actual adverse movement in price
   double adverse_movement = last_entry_price - bid;
...
/// when this algorythim apply and has only single positions
...
void CloseWhenSinglePositionProfit(Stats &s) {
   ...
   if(s.floatingProfit >= profitLotsize) {
        ...
        CloseAllBuys();
        if(AutoRestart) Print("AutoRestart pending.");
   }
}

//--- Example function: log trade on position close
//--- This functionjust use for log in the future will connect API
